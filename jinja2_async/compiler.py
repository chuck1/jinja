...
class CodeGenerator(NodeVisitor):
    ...
    def visit_Template(self, node, frame=None):
        ...
        from jinja2_async.runtime import __all__ as exported
        self.writeline('from __future__ import %s' % ', '.join(code_features))
        self.writeline('from jinja2_async.runtime import ' + ', '.join(exported))

        if self.environment.is_async:
            self.writeline('from jinja2_async.asyncsupport import auto_await, '
